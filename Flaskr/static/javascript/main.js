...
function txtupdate(){
    setTimeout (() => {header.remove(); header.innerHTML = "‎ The Perfect Job Description."; document.getElementById('wel').appendChild(header)}, 500)

    setTimeout (() => {header.remove(); header.innerHTML = "‎ Resume Points."; document.getElementById('wel').appendChild(header)}, 5000)

    setTimeout (() => {header.remove(); header.innerHTML = "‎ What You Know."; document.getElementById('wel').appendChild(header)}, 10000);
...
output_activator.addEventListener('click', () => {
    ...
    .then(response => {
        if (response.ok) {
        console.log('Resetable updated successfully');
        } else {
...
document.addEventListener("DOMContentLoaded", function(){
    fetch('/get_resetable').then(function(response) {
